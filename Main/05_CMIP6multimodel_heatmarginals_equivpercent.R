...
equivpercent <- array(rep(NaN, length(modelinds)*3), c(length(modelinds), 3))
...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    equivpercent[modelpos, 1] <- ecdf(data_1deg[,1])(varbaselinethreshold)
    equivpercent[modelpos, 2] <- ecdf(data_2deg[,1])(varbaselinethreshold)
    equivpercent[modelpos, 3] <- ecdf(data_3deg[,1])(varbaselinethreshold)
  }
}

#export equivalent percentiles information
write.csv(equivpercent, file = paste0(resultsdir, '/CMIP6multimodel_', vartype, '-equivalentpercent-JJA-allmodels.csv'), row.names = modelnames)
