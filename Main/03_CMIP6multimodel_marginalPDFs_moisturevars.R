...
#jpeg(paste0(figdir, '/CMIP6multimodel_', mvartype, '-marginalPDFs_withbaseline_anom-JJA-allmodels_demp_legend.jpeg'), width = 10/1.4, height = 12/1.4, units = 'in', res = 300)

jpeg(paste0(figdir, '/CMIP6multimodel_', mvartype, '-marginalPDFs_withbaseline_anom_demp_JJA-allmodels_units.jpeg'), width = 8*2, height = 4*2, units = 'in', res = 300)
...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    #xpoints <- c(max(data_1deg[,2]), max(data_2deg[,2]), max(data_3deg[,2]))
    #ypoints <- c(0, 0, 0)
    ...
    plot(make_marg_empbase, from=xlim[1], to=xlim[2], xlim = c(xlim[1], xlim[2]), ylim = c(0,yuplim), col = 'black')
    title(main = paste0(currentmodel), line = 0.5)
    plot(make_marg_emp1, from=xlim[1], to=xlim[2], xlim = c(xlim[1], xlim[2]), col = 'orange', ylim = c(0,yuplim), add=TRUE)
    plot(make_marg_emp2, from=xlim[1], to=xlim[2], xlim = c(xlim[1], xlim[2]), col = 'red', ylim = c(0,yuplim), add=TRUE)
    plot(make_marg_emp3, from=xlim[1], to=xlim[2], xlim = c(xlim[1], xlim[2]), col = 'purple', ylim = c(0,yuplim), add=TRUE)
...
#legend(x = 'bottom', inset = c(0, -0.13), legend = c('1 Degree', '2 Degrees', '3 Degrees'), col = c('orange', 'red', 'purple'), ncol = 3, lty = 1, xpd = TRUE)
