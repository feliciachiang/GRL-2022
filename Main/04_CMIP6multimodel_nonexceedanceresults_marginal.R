...
nonexceeds <- array(rep(NaN, length(modelinds)*3), c(length(modelinds), 3))
...
for (modelpos in 1:length(modelinds)) {
  ...
  varbaselinethreshold <- quantile(as.vector(varbaseline), percentile)
  var1dthreshold <- quantile(as.vector(vardata[,,1]), percentile)
  var2dthreshold <- quantile(as.vector(vardata[,,2]), percentile)
  var3dthreshold <- quantile(as.vector(vardata[,,3]), percentile)
  ...
  nonexceeds[modelpos, 1] <- MESS::auc(cond_dens_emp1_x[cond_dens_emp1_x < var1dthreshold], cond_dens_emp1_y[cond_dens_emp1_x < var1dthreshold], type = 'spline')/MESS::auc(cond_dens_emp1_x, cond_dens_emp1_y, type = 'spline')
  nonexceeds[modelpos, 2] <- MESS::auc(cond_dens_emp2_x[cond_dens_emp2_x < var2dthreshold], cond_dens_emp2_y[cond_dens_emp2_x < var2dthreshold], type = 'spline')/MESS::auc(cond_dens_emp2_x, cond_dens_emp2_y, type = 'spline')
  nonexceeds[modelpos, 3] <- MESS::auc(cond_dens_emp3_x[cond_dens_emp3_x < var3dthreshold], cond_dens_emp3_y[cond_dens_emp3_x < var3dthreshold], type = 'spline')/MESS::auc(cond_dens_emp3_x, cond_dens_emp3_y, type = 'spline')
  
}

#save nonexceeds as csv
write.csv(nonexceeds, paste0(resultsdir, '/CMIP6multimodel_', mvartype, vartype, '-nonexceeding-degree75th-JJA-allmodels.csv'), row.names= FALSE)
