...
#load copula family, goodness of fit pvale, and copula parameters
nc <- nc_open(paste0(resultsdir, '/CMIP6multimodel_', mvartype, vartype, '-copparams-JJA-allmodels.nc'))
copfitdata <- ncvar_get(nc, nc$var[["copparams"]])  
nc_close(nc)
...
jpeg(paste0(figdir, '/CMIP6multimodel_', mvartype, vartype, '-conditionalPDFs_withbaseline_anom-JJA-allmodels_col_yaxis.jpeg'), width = 3.5/1.4, height = 12/1.4, units = 'in', res = 300)
...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    fittedcopulabaseline <- copulaFromFamilyIndex(copfitdata[modelpos, 1, 1], copfitdata[modelpos, 1, 2], copfitdata[modelpos, 1, 3])
    ...
    fittedcopula1 <- copulaFromFamilyIndex(copfitdata[modelpos, 2, 1], copfitdata[modelpos, 2, 2], copfitdata[modelpos, 2, 3])
    #use imported make_emp_cpdf function to estimate conditional PDF at the given percentile
    ...
    fittedcopula2 <- copulaFromFamilyIndex(copfitdata[modelpos, 3, 1], copfitdata[modelpos, 3, 2], copfitdata[modelpos, 3, 3])
    #use imported make_emp_cpdf function to estimate conditional PDF at the given percentile
    ...
    fittedcopula3 <- copulaFromFamilyIndex(copfitdata[modelpos, 4, 1], copfitdata[modelpos, 4, 2], copfitdata[modelpos, 4, 3])
    #use imported make_emp_cpdf function to estimate conditional PDF at the given percentile
    ...
    plot(cond_dens_empbase, from=xlowlim, to=xhilim, xlim = c(xlowlim, xhilim), ylim = c(ylowlim, yhilim), col = 'green', yaxt = 'n')
    
    #for pr and tasmax
    #axis(2, at = c(0, 0.25, 0.5, 0.75, 1.2), labels = c(0, 0.25, 0.5, 0.75, 1.2))
    
    #for mrsos and tasmax
    # if (between(yhilim, 0.9, 1.1)) {
    #   axis(2, at = c(0, 0.5, 1.5), labels = c(0, 0.5, 1.5))
    # } else {
    #   axis(2, at = c(0, 0.5, 1, 1.5), labels = c(0, 0.5, 1, 1.5))
    # }
    
    #for pr and vpd
    axis(2, at = c(0, 0.3, 0.6, 0.9, 1.2, 1.5), labels = c(0, 0.3, 0.6, 0.9, 1.2, 1.5))
