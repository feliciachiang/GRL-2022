...
copfamily <- array(rep(NaN, length(modelinds)*length(endmonthinds)*2), c(length(endmonthinds), length(modelinds), 2))
names(dim(copfamily)) <- c('endmonth', 'model', 'warmingdegree')
gofpval <- array(rep(NaN, length(modelinds)*length(endmonthinds)*2), c(length(endmonthinds), length(modelinds), 2))
names(dim(gofpval)) <- c('endmonth', 'model', 'warmingdegree')
copparams <- array(rep(NaN, length(modelinds)*length(endmonthinds)*2*3), c(length(endmonthinds), length(modelinds), 2, 3))
...
jpeg(paste0(figdir, '/CMIP6multimodel_', mvartype, vartype, '-conditionalPDFs_anom-JJA-allmodels_forEMEarth_legend.jpeg'), width = 8*2, height = 4*2, units = 'in', res = 300)
par(mfrow = c(3, 4),
    oma = c(5,4,0,0) + 0.1,
    mar = c(0,0,4,3) + 0.1)
...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    nc <- nc_open(paste0(resultsdir, currentmodel, '_', mvartype, '_', maggtype, '_', savestr, '_EMEarthperiods_first5ens.nc'))
    mvardata <- ncvar_get(nc, nc$var[[mvartype]])  
    #degrees <- nc$dim$degree$vals
    nc_close(nc)

    #import heat variable data
    nc <- nc_open(paste0(resultsdir, currentmodel, '_', vartype, '_', aggtype, '_', savestr, '_EMEarthperiods_first5ens.nc'))
    vardata <- ncvar_get(nc, nc$var[[vartype]])
    nc_close(nc)

    #------------------------------------------------------------------------------
    #find seasonal averages from the baseline (1850-1899)
    baseline_seasonalmean <- mean(vardata[,,1])
    ...
    varbaselinethreshold <- quantile(vardata[,,1], percentile)
    
    #use negative precipitation in order to have positive correlation for x and y
    data_baseline <- data.frame(as.vector(vardata[,,1]), as.vector(-mvardata[,,1]))
    data_1deg <- data.frame(as.vector(vardata[,,2]), as.vector(-mvardata[,,2]))
    
    #import baseline 1850-1899 data for the relevant season
    baselinedata <- mvardata[,,1]
    ...
    # xlim = c(min(vardata[,,1])-1, max(vardata[,,2]) + 1)
    # ylim = c(min(-mvardata[,,1])*1.05, max(-mvardata[,,2])*0.95)
    # 
    ...
    # jpeg(paste0(figdir, '/', currentmodel, '_', mvartype, vartype, '-', savestr, '-scatter_EM-Earthtimeperiod.jpeg'), width = 4.5*1.25, height = 4*1.25, units = 'in', res = 300)
    # 
    ...
    # dev.off()
    # 
    # #plot marginals for current variable pair
    # 
    # #create empirical estimation of marginal distribution of variables at 1950-1969
    # make_marg_empbase <-  function(x) demp(x, as.vector(data_baseline[,1]))
    # 
    # #marginal at 2000-2019
    ...
    # 
    # #plots
    # jpeg(paste0(figdir, '/', currentmodel, '_', vartype, '-', savestr, '-marginalPDF_EMEarthtimeperiod.jpeg'), width = 4.5*1.25, height = 4*1.25, units = 'in', res = 300)
    # 
    ...
    ylowlim <- 0
    yhilim <- max(cond_base_ymax, cond_emp1_ymax)
    yhilim <- yhilim*1.15
    
    #jpeg(paste0(figdir, '/', currentmodel, '_', mvartype, vartype, '-', savestr, '-conditionalPDFs_anom_EM-Earthtimeperiod.jpeg'), width = 4.5*1.25, height = 4*1.25, units = 'in', res = 300)
    
    
    #plot conditional PDFs for 1, 2, 3, and 4 degrees of warming
    jpeg(paste0(figdir, '/CMIP6multimodel_', mvartype, vartype, '-conditionalPDFs_anom-JJA-allmodels_forEMEarth_legend.jpeg'), width = 9, height = 8, units = 'in', res = 300)
    ...
    plot(cond_dens_empbase, from=xlowlim, to=xhilim, xlim = c(xlowlim, xhilim), ylim = c(ylowlim, yhilim), col = 'green')
    ...
    #export plot 
    legend(x = 'bottom', inset = c(0, -0.25), legend = c('1950-1969', '2000-2019'), col = c('green', 'orange'), ncol = 2, lty = 1, xpd = TRUE)
...
#legend(x = 'top', inset = 0, legend = c('Baseline', '1 Degree'), col = c('green', 'orange'), ncol = 2, lty = 1, xpd = TRUE)
...
ncvar_put(ncnew, var_temp, gofpval, start=c(1,1,1), count=c(nx,ny,2))
...
ncvar_put(ncnew, var_temp, copparams, start=c(1,1,1,1), count=c(nx,ny,2, 3))
