...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    snr[modelpos] <- (mean(data_1deg[,1])-mean(data_baseline[,1]))/(sd(data_baseline[,1]))
...
write.csv(snr, file = paste0(resultsdir, '/CMIP6multimodel_', vartype, '-snr-JJA-allmodels_1deg.csv'), row.names = modelnames)
