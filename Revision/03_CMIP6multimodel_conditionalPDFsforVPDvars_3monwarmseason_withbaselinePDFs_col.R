...
#         -June-Aug
# Output: Conditional plots for variables relevant for VPD (ea, es, lhflx, shflx, EF)
...
#------------------------------------------------------------------------------#
#precipitation x VPD-related variables----------------------------------------
mvartype <- 'pr'
mvarstring <- 'Pr'
...
#latent heat flux (hfls), sensible heat flux (hfss), saturated vapor pressure (es), actual vapor pressure (ea) 
vartype <- 'hfls'
...
for (modelpos in 1:length(modelinds)) {
  ...
  for (endmonthpos in 1:length(endmonthinds)) {
    ...
    #xlowlim <- -20   #for lhflx
    #xhilim <- 30     #for lhflx
    
    xlowlim <- -20    #for shflx
    xhilim <- 25      #for shflx
